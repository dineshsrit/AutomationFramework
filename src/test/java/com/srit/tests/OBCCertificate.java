...
public class OBCCertificate extends BaseTest{


    String obc=null;

    Xls_Reader reader=new Xls_Reader(FrameworkConstants.getExcelFilepath());
    String sheetname="obc";
    @Test
    public void applyforOBCServices()
    {
        try
        {
            ...
            ObcPage sp= cp.click_obcCertificate();
            sp.click_Proceed();
    ...
    public void forwardtoTahsilbyRI()
    {
        try
        {
            ...
            HandlePop.getHandlePop();
            String ConfirmVerificationApplicationNo=sp.getobcVerificationNo().substring(2);
            System.out.println(ConfirmVerificationApplicationNo);
            Assert.assertEquals(obc,ConfirmVerificationApplicationNo );
            ...
            Assert.assertEquals(obc, confirm, "obc No didnt match");
            sp.clickdaclose();
            sp.clickUser();
            Thread.sleep(1000);
            sp.clickLogout();
